...
elseif ($doGuess) {
    $tries=1;
    if($guess_orig===$number) {
        $res = "Correct!!";
    } elseif ($guess_orig>$number) {
        $res = "Too high!!";
    } else {
        $res = "Too low!!";
    }
...
<form>
<input type="text" name="guess_orig">
    <input type="hidden" name="number" value="<?= //$number ?>">
    <input type="submit" name="doGuess" value="Make a guess_orig">
    <input type="submit" name="doInit" value="Start over">
    <input type="submit" name="doCheat" value="Cheat">
</form>

<?php //if($doGuess) : ?>
<p>Your guess_orig <?= //$guess_orig ?> is <b><?= //$res ?></b></p>
<?//php endif; ?>
<?//php if($doCheat) : ?>
